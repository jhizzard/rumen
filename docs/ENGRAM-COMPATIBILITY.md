# Engram compatibility contract (Rumen v0.1)

Rumen v0.1 is **tightly coupled** to the schema exposed by [Engram](https://github.com/jhizzard/engram). Future Rumen versions may abstract this behind an adapter layer, but v0.1 reads Engram's tables and calls Engram's SQL functions directly. If your memory store does not follow the Engram schema, Rumen v0.1 will not work.

This document is the frozen contract for v0.1. Any change here is a breaking change and requires a minor version bump.

## Why coupled?

See the Podium lessons referenced in the RUMEN pre-deployment checklist: Rumen is intentionally small (~200 LOC in v0.1) and uses raw `pg` rather than Prisma. Introducing an adapter layer would roughly double the surface area for no v0.1 benefit. Engram is the first and currently only consumer, so we couple now and abstract later if a second memory store needs to plug in.
...
v0.1 never writes to `memory_items`. Rumen is strictly a reader of Engram's memory tables.
...
- Rumen calls this with `query_embedding := NULL` in v0.1. Engram's implementation must fall back to keyword-only (tsvector) matching when the embedding argument is NULL. v0.2 will start passing a real embedding.
...
If your Engram fork returns additional columns, Rumen will ignore them — the column list above is the minimum.
...
If Engram renames a column or changes a signature:
...
v0.5+ is the likely home for an adapter abstraction: `RumenMemoryStore` interface with `fetchRecentSessions`, `searchRelated`, etc. Until then, if you need to run Rumen on top of a non-Engram store, fork `extract.ts` and `relate.ts` rather than trying to configure around them.
